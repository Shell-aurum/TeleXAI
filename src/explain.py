...
class TeleXAIExplainer:
    ...
    def get_lime_explanation_data(self, input_data: pd.DataFrame, num_features: int = 8):
        """
        Same LIME explanation as get_lime_explanation, but returns the raw
        (condition, weight) pairs instead of rendered HTML. LIME's bundled
        HTML visualization embeds its own webpack/D3 bundle and depends on
        script execution inside whatever iframe renders it, which is fragile
        across browsers and sandboxing setups. Returning structured data
        lets the frontend render the explanation itself, with no dependency
        on that bundle running correctly.
        """
        input_data = input_data[self.feature_cols]
        row_values = input_data.iloc[0].values
